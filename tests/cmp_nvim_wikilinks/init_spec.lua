...
describe("cmp_wikilinks", function()
	describe("._get_completion_pattern", function()
		it("returns nil if the cursor is not within a full or partial link", function()
			...
			assert.is_nil(
				lib._get_completion_pattern("hello [[open-link/to-somewhere", 3),
		...
		it("returns the pattern for open links at the end of the line", function()
			...
			assert.equals(
				"f",
				lib._get_completion_pattern("see [[not this one]] [[foo", 25),
				"open link at end of line with preceeding link"
			)
			assert.equals(
				"foo",
				lib._get_completion_pattern("see [[not this one]] [[foo", 27),
				"end of open link at end of line with preceeding link"
			)

			assert.equals(
				"foo/bar",
				lib._get_completion_pattern("see [[foo/bar", 14),
				"end of open link at end of line (with path)"
			)
			assert.equals(
				"foo/ba",
				lib._get_completion_pattern("see [[foo/bar", 13),
				"open link at end of line (with path)"
			)
			assert.equals(
				"foo/b",
				lib._get_completion_pattern("see [[not this one]] [[foo/bar", 29),
				"open link at end of line with preceeding link (with path)"
			)
			assert.equals(
				"foo/bar",
				lib._get_completion_pattern("see [[not this one]] [[foo/bar", 31),
		...
		it("returns the pattern for open links in the middle of the line", function()
			...
			assert.equals(
				"foo b",
				lib._get_completion_pattern("see [[foo but not [[bar]]", 12),
				"open link mid-line with spaces"
			)

			assert.equals(
				"foo/bar",
				lib._get_completion_pattern("see [[foo/bar but not [[bar]]", 14),
				"open link mid-line (with path)"
			)
			assert.equals(
				"foo/bar b",
				lib._get_completion_pattern("see [[foo/bar but not [[bar]]", 16),
		...
		it("returns the pattern for links", function()
			assert.equals(
				"foo",
				lib._get_completion_pattern("see [[foo]] or perhaps [[bar]] i dunno", 10),
				"full first link"
			)
			assert.equals(
				"bar",
				lib._get_completion_pattern("see [[foo]] or perhaps [[bar]] i dunno", 29),
				"full subsequent link (with path)"
			)
			assert.equals(
				"bar",
				lib._get_completion_pattern("see [[foo but not [[bar]]", 24),
				"full link following open link"
			)

			assert.equals(
				"foo/bar",
				lib._get_completion_pattern("see [[foo/bar]] or perhaps [[bar]] i dunno", 14),
				"full first link (with path)"
			)
			assert.equals(
				"bar/bar",
				lib._get_completion_pattern("see [[foo]] or perhaps [[bar/bar]] i dunno", 33),
				"full subsequent link (with path)"
			)
			assert.equals(
				"bar/bar",
				lib._get_completion_pattern("see [[foo but not [[bar/bar]]", 28),
	...
	describe("._find_completion_items", function()
		...
		it("supports matching against multiple glob suffixes", function()
			local glob_pattern_calls = {}

			local globpath = function(_, pattern)
				table.insert(glob_pattern_calls, pattern)
				if pattern == "foo*" then
					return {
						"/home/user/vault/foobar",
						"/home/user/vault/foobaz",
					}
				elseif pattern == "foo*/*" then
					return {
						"/home/user/vault/foobar/index",
						"/home/user/vault/foobar/a",
					}
				elseif pattern == "foo*/*/*" then
					return {
						"/home/user/vault/foobar/a/okay",
						"/home/user/vault/foobar/a/seemsgood",
					}
				else
					error('wrong pattern "' .. pattern .. '" passed to glob')
				end
			end
			...
			local opts = { paths = paths, globpath = globpath, glob_suffixes = { "*", "*/*", "*/*/*" } }

			local completed_items = lib._find_completion_items("foo", opts)
			assert.same({ "foo*", "foo*/*", "foo*/*/*" }, glob_pattern_calls)
			assert.same({
				{ label = "foobar", path = "/home/user/vault/foobar" },
				{ label = "foobaz", path = "/home/user/vault/foobaz" },
				{ label = "foobar/index", path = "/home/user/vault/foobar/index" },
				{ label = "foobar/a", path = "/home/user/vault/foobar/a" },
				{ label = "foobar/a/okay", path = "/home/user/vault/foobar/a/okay" },
				{ label = "foobar/a/seemsgood", path = "/home/user/vault/foobar/a/seemsgood" },
			}, completed_items)
		end)
	end)
